...
public class AVPNReloadCommand extends CommandHandler {
    ...
    protected void onExecute(long elapsedMilliseconds) {
        ...
        if (!configFile.exists()) {
            try (InputStreamReader reader = new InputStreamReader(ServiceLocator.getService(Plugin.class).getResource("config.yml"));
            ...
            if (Config.sendErrors) {
                ThreadUtil.submit(new Runnable() {
                    public void run() {
                        ...
                        plugin.swapExceptionHandlers(new RollbarExceptionHandler(Config.ROLLBAR_KEY, "production", plugin.getDescription().getVersion(), plugin.getServerId(), plugin.getName()));
                    }
                });
            } else {
                ThreadUtil.submit(new Runnable() {
        ...
        MessagingLoader.loadMessaging("avpn", Bukkit.getServerName(), ServiceLocator.getService(BasePlugin.class).getServerId(), SenderType.SERVER);
