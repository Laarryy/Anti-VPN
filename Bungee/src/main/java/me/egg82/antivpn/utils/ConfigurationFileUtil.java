...
public class ConfigurationFileUtil {
    ...
    public static void reloadConfig(Plugin plugin) {
        ...
        if (debug) {
            plugin.getProxy().getConsole().sendMessage(new TextComponent(getHeading() + ChatColor.YELLOW + "Debug " + ChatColor.WHITE + "enabled"));
        }
        ...
        if (debug) {
            plugin.getProxy().getConsole().sendMessage(new TextComponent(getHeading() + ChatColor.YELLOW + "Source cache time: " + ChatColor.WHITE + sourceCacheTimeUnit.get().toMillis(sourceCacheTimeLong.get()) + " millis"));
        }
        ...
        if (debug) {
            plugin.getProxy().getConsole().sendMessage(new TextComponent(getHeading() + ChatColor.YELLOW + "Memory cache time: " + ChatColor.WHITE + cacheTimeUnit.get().toMillis(cacheTimeLong.get()) + " millis"));
        }
        ...
        for (Iterator<String> i = sources.iterator(); i.hasNext();) {
            ...
            if (!config.getNode("sources", source, "enabled").getBoolean()) {
                if (debug) {
                    plugin.getProxy().getConsole().sendMessage(new TextComponent(getHeading() + ChatColor.DARK_RED + source + " is disabled. Removing."));
                }
                i.remove();
                continue;
            }
            ...
            if ((source.equalsIgnoreCase("iphub")
                ...
                if (debug) {
                    plugin.getProxy().getConsole().sendMessage(new TextComponent(getHeading() + ChatColor.DARK_RED + source + " requires a key which was not provided. Removing."));
                }
                i.remove();
            }
        }
        ...
        if (debug) {
            for (String source : sources) {
                plugin.getProxy().getConsole().sendMessage(new TextComponent(getHeading() + ChatColor.YELLOW + "Added source: " + ChatColor.WHITE + source));
            }
        }
        ...
        if (debug) {
            for (String ip : ignoredIps) {
                plugin.getProxy().getConsole().sendMessage(new TextComponent(getHeading() + ChatColor.YELLOW + "Ignoring IP: " + ChatColor.WHITE + ip));
            }
        }
        ...
        if (debug) {
            plugin.getProxy().getConsole().sendMessage(new TextComponent(getHeading() + ChatColor.YELLOW + "API threads: " + ChatColor.WHITE + cachedValues.getThreads()));
            plugin.getProxy().getConsole().sendMessage(new TextComponent(getHeading() + ChatColor.YELLOW + "Using Redis: " + ChatColor.WHITE + (cachedValues.getRedisPool() != null)));
            plugin.getProxy().getConsole().sendMessage(new TextComponent(getHeading() + ChatColor.YELLOW + "Using RabbitMQ: " + ChatColor.WHITE + (cachedValues.getRabbitConnectionFactory() != null)));
            plugin.getProxy().getConsole().sendMessage(new TextComponent(getHeading() + ChatColor.YELLOW + "SQL type: " + ChatColor.WHITE + cachedValues.getSQLType().name()));
        }
    }
