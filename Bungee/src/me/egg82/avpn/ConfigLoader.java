...
public class ConfigLoader {
	...
	public static Configuration getConfig(String resourcePath, String configFileName) {
		...
		if (!configFile.exists()) {
			try (InputStreamReader reader = new InputStreamReader(ServiceLocator.getService(Plugin.class).getResourceAsStream(resourcePath)); BufferedReader in = new BufferedReader(reader); FileWriter writer = new FileWriter(configFile); BufferedWriter out = new BufferedWriter(writer)) {
				String line = null;
		...
		try {
			root = loader.load(ConfigurationOptions.defaults().setHeader("Comments are gone because update :(. Click here for new config + comments: https://www.spigotmc.org/resources/anti-vpn-bungee.58716/"));
		} catch (Exception ex) {
	...
	private static void to20(ConfigurationNode config) {
		...
		// Add ignore
		config.getNode("ignore").setValue(Arrays.asList(new String[] {
			"127.0.0.1",
			"localhost",
			"::1"
		}));
