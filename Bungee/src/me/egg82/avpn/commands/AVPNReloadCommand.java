...
public class AVPNReloadCommand extends AsyncCommandHandler {
	...
	protected void onExecute(long elapsedMilliseconds) {
		...
		if (!configFile.exists()) {
			try (InputStreamReader reader = new InputStreamReader(ServiceLocator.getService(Plugin.class).getResourceAsStream("config.yml")); BufferedReader in = new BufferedReader(reader); FileWriter writer = new FileWriter(configFile); BufferedWriter out = new BufferedWriter(writer)) {
				while (in.ready()) {
		...
		MessagingLoader.loadMessaging(ServiceLocator.getService(Plugin.class).getDescription().getName(), null, ServiceLocator.getService(BasePlugin.class).getServerId(), SenderType.PROXY);
