...
public class ConfigLoader {
    ...
    public static Configuration getConfig(String resourcePath, String configFileName) {
        File dataDir = new File(ServiceLocator.getService(BasePlugin.class).getDescription().getSource().orElse(null).getParent().toFile(),
            ServiceLocator.getService(BasePlugin.class).getPluginName());
        ...
        if (!configFile.exists()) {
            try (InputStreamReader reader = new InputStreamReader(ServiceLocator.getService(BasePlugin.class).getClass().getClassLoader().getResourceAsStream(resourcePath));
    ...
    private static void conformVersion(ConfigurationLoader<ConfigurationNode> loader, ConfigurationNode config, String configFileName) {
        ...
        if (config.getNode("version").getDouble() != oldVersion) {
            ...
            File backupFile = new File(dataDir, configFileName + ".bak");
