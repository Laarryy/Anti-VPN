...
public class AVPNScoreCommand extends AsyncCommandHandler {
    ...
    protected void onExecute(long elapsedMilliseconds) {
        ...
        ThreadUtil.submit(new Runnable() {
            public void run() {
                ...
                sender.sendMessage(TextComponent.of("Sleeping for one minute..", TextColor.YELLOW).content());
                try {
                    Thread.sleep(60000L);
                } catch (Exception ex) {
                ...
                sender.sendMessage(TextComponent.of("Score complete!", TextColor.GREEN).content());
            }
        });
    }
    ...
    private void test(String source, String vpnName, Set<String> ips, boolean flipResult) {
        ...
        sender.sendMessage(TextComponent.of("Scoring against " + vpnName + "..", TextColor.YELLOW).content());
        for (String ip : ips) {
            try {
                Thread.sleep(5000L);
            } catch (Exception ex) {
        ...
        if (error > 0) {
            sender.sendMessage(TextComponent.of("Source error: " + format.format((error / ips.size()) * 100.0d) + "%", TextColor.YELLOW).content());
        }
        sender.sendMessage(TextComponent.of(vpnName + " score: " + format.format((good / (ips.size() - error)) * 100.0d) + "%", TextColor.YELLOW).content());
    }
}
