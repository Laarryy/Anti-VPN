...
public interface LocalizedCommandSender<M extends LocalizedCommandSender<M, B>, B> {
    ...
    default void sendMessage(@NotNull MessageKey key) { sendMessage(getLocalizedText(key)); }

    default void sendMessage(@NotNull MessageKey key, String... placeholders) { sendMessage(getLocalizedText(key, placeholders)); }

    default void sendMessage(@NotNull MessageKey key, @NotNull Map<String, String> placeholders) { sendMessage(getLocalizedText(key, placeholders)); }
    ...
    default @NotNull Component getComponent(@NotNull MessageKey key, @NotNull Map<String, String> placeholders) { return getComponent(getLocalizedText(key, placeholders)); }
