...
public class ClearDataSQLiteCommand extends Command {
    ...
    protected void onExecute(long elapsedMilliseconds) {
        ...
        mainQuery = sql.parallelQuery("DELETE FROM `antivpn` WHERE CURRENT_TIMESTAMP > DATETIME(`created`, ?);", "+" + Math.floorDiv(Config.sourceCacheTime, 1000L) + " seconds");
        consensusQuery = sql.parallelQuery("DELETE FROM `antivpn_consensus` WHERE CURRENT_TIMESTAMP > DATETIME(`created`, ?);", "+" + Math.floorDiv(Config.sourceCacheTime, 1000L) + " seconds");
    }
