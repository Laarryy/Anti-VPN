...
public class CreateTablesMySQLCommand extends Command {
    ...
    protected void onExecute(long elapsedMilliseconds) {
        ...
        consensusQuery = sql.query("SELECT COUNT(*) FROM information_schema.tables WHERE table_schema=? AND table_name='antivpn_consensus';", config.getNode("sql", "mysql", "database").getString());
        ...
        consensusQueueQuery = sql.query("SELECT COUNT(*) FROM information_schema.tables WHERE table_schema=? AND table_name='antivpn_consensus_queue';",
            config.getNode("sql", "mysql", "database").getString());
    }
    ...
    private void onSQLData(SQLEventArgs e) {
        if (e.getUuid().equals(mainQuery)) {
            ...
            sql.query("CREATE TABLE `antivpn` (" + "`ip` VARCHAR(45) NOT NULL," + "`value` BOOLEAN NOT NULL," + "`created` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP" + ");");
            ...
            sql.query("CREATE TABLE `antivpn_consensus` (" + "`ip` VARCHAR(45) NOT NULL," + "`value` DOUBLE NOT NULL," + "`created` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP" + ");");
            sql.query("ALTER TABLE `antivpn_consensus` ADD UNIQUE (`ip`);");
        } else if (e.getUuid().equals(queueQuery)) {
            if (e.getData().data.length > 0 && e.getData().data[0].length > 0 && ((Number) e.getData().data[0][0]).intValue() != 0) {
                return;
            }

            sql.query("CREATE TABLE `antivpn_queue` (" + "`ip` VARCHAR(45) NOT NULL," + "`value` BOOLEAN NOT NULL," + "`created` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,"
                + "`updated` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP" + ");");
            sql.query("ALTER TABLE `antivpn_queue` ADD UNIQUE (`ip`);");
            ...
            sql.query("CREATE TABLE `antivpn_consensus_queue` (" + "`ip` VARCHAR(45) NOT NULL," + "`value` DOUBLE NOT NULL," + "`created` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,"
                + "`updated` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP" + ");");
            finalQuery = sql.query("ALTER TABLE `antivpn_consensus_queue` ADD UNIQUE (`ip`);");
    ...
    private void onSQLError(SQLEventArgs e) {
        if (!e.getUuid().equals(mainQuery) && !e.getUuid().equals(consensusQuery) && !e.getUuid().equals(queueQuery) && !e.getUuid().equals(consensusQueueQuery) && !e.getUuid().equals(finalQuery)) {
            return;
        }
