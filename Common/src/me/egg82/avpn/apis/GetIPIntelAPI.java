...
public class GetIPIntelAPI implements IFetchAPI {
    ...
    public Optional<Boolean> getResult(String ip) {
        JSONObject json = WebUtil.getJson("https://check.getipintel.net/check.php?ip=" + ip + "&contact="
            + ServiceLocator.getService(Configuration.class).getNode("sources", "getipintel", "contact").getString("") + "&format=json&flags=b");
        if (json == null) {
